...
* `onMount(store, cb)`: shortcut to use both `onStart` and `onStop`. We recommend to always use `onMount` instead of `onStart + onStop`, because it has a short delay to prevent undesirable behavior.
