...
function deactivate(signal) {
  ...
  if (!deactivateLevel++) shouldInvalidate = false
  for (let link = signal[_firstSource]; link; link = link.ns) {
  ...
  if (!--deactivateLevel && shouldInvalidate && !batchLevel) {
    ++globalVersion
  }
}
...
function sync() {
  ...
  const subVersion = globalVersion
  for (let link of linksToSubscribers) {
    const signal = link.s
    if (!signal) continue
    if (signal[_updated] === subVersion) {
      try {
...
export { NONE, Signal, WritableSignal, batch }
